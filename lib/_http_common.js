...
function sliceHeaders(headers) {
  var tmp = new Array(headers.length >>> 1);
  var whole = headers[0];
  for (var i = 1, j = 0; i < headers.length; i += 4, j += 2) {
    tmp[j] = whole.slice(headers[i], headers[i + 1]);
    tmp[j + 1] = whole.slice(headers[i + 2], headers[i + 3]);
  }
  return tmp;
}
